...
| ----- | ----- | ----- | ----- | ----- | ----- |
| `tmo ble` | Various **B**luetooth **L**ow **E**nergy (BLE) commands.<br><br>Only available with tmo_shell version 1.10.x and later. Upgrade [here](09-Upgrading-your-IoT-Developer-Kit.md). | 1. smp<br>2. adv | 1. BLE SMP Controls<br>2. BLE advertisement test controls | | 1. `tmo ble adv ibeacon` |
| `tmo buzzer` | Buzzer test.<br><br>Only available with tmo_shell version 1.10.x and later. Upgrade [here](09-Upgrading-your-IoT-Developer-Kit.md). | 1. jingle<br>2. ramp<br>3.tone | 1. Play TMO jingle<br>2. Play ramp tune<br>3. Play a tone for a time. | 1. Not applicable<br>2. Not applicable<br>3. &lt;freq in Hz&gt; &lt;time in msecs&gt; | 1. `tmo buzzer jingle`<br>2. `tmo buzzer ramp`<br>3. `tmo buzzer tone <freq in Hz> <time in msecs>` |
| `tmo certs` | CA cert commands. Load, list, or dump certificates of authority on to your kit.<br><br>Only available with tmo_shell version 1.10.x and later. Upgrade [here](09-Upgrading-your-IoT-Developer-Kit.md). | 1. list<br>2. dump<br>3. load | 1. List certificates.<br>2. Dump certificates<br>3.Load certificates | | 1. `tmo certs list`<br>2. `tmo certs dump` | 
| `tmo dfu` | Device Firmware (FW) Updates | 1. base_url (v1.10.x)<br>2. download (v1.10.x)<br>3. iface (v1.10.x)<br>4. settings (v1.10.x)<br>5. update<br>6. version | 1. Set firmware download base URL. (Only available with tmo_shell version 1.10.x and later. Upgrade here.)<br>2. Download firmware. (Only available with tmo_shell version 1.10.x and later. Upgrade here.)<br>3. Set firmware download iface. (Only available with tmo_shell version 1.10.x and later. Upgrade here.)<br>4. Print DFU settings.<br>5. Update firmware. (Only available with tmo_shell version 1.10.x and later. Upgrade here.)<br>6. Get current firmware version. | &lt;iface&gt; | 1. `tmo dfu version 1`<br> 2. `tmo dfu update 2 0`<br>3. `tmo dfu download 2` |
| `tmo dns` | Performs a DNS lookup. | None | Not applicable | &lt;dev&gt;<br>&lt;hostname&gt; | `tmo dns 2 t-mobile.com` |
| `tmo ifaces` | File commands.<br>Only available with tmo_shell version 1.10.x and later. Upgrade here. | 1. cp<br>2. ll<br>3. mv <br>4.sha1 | 1. Copy a file.<br>2. Detailed file list.<br>3. Move a file. <br>4. Compute a file SHA1. | | 1. `tmo file cp <source file> <destination file>`<br>2. `tmo file ll <folder>` | 
| `tmo http` | Get http URL | None | Not applicable | Not applicable | `tmo http 2 http://www.t-mobile.com` |
<br>
